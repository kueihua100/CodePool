...
static FrontendConfig frontendArray[FRONTEND_MAX];
...
inline void initFrontendConfig() {
    ...
    frontendArray[DVBT].isSoftwareFe = false;
    ...
    FrontendAtscSettings atscSettings{
            .frequency = 195000000,
            .modulation = FrontendAtscModulation::AUTO,
    };
    frontendArray[ATSC].type = FrontendType::ATSC;
    frontendArray[ATSC].settings.atsc(atscSettings);
    vector<FrontendStatusType> atsc_fetypes;
    atsc_fetypes.push_back(FrontendStatusType::DEMOD_LOCK);
    FrontendStatus atsc_status;
    atsc_status.isDemodLocked(true);
    vector<FrontendStatus> atsc_vstatuses;
    atsc_vstatuses.push_back(atsc_status);
    frontendArray[ATSC].tuneStatusTypes = atsc_fetypes;
    frontendArray[ATSC].expectTuneStatuses = atsc_vstatuses;
    frontendArray[ATSC].isSoftwareFe = false;

    //ATSC3
    FrontendAtsc3Settings atsc3Settings{
            .frequency = 69000000,
            .bandwidth = FrontendAtsc3Bandwidth::BANDWIDTH_6MHZ,
            .demodOutputFormat = FrontendAtsc3DemodOutputFormat::ATSC3_LINKLAYER_PACKET,
    };
    frontendArray[ATSC3].type = FrontendType::ATSC3;
    frontendArray[ATSC3].settings.atsc3(atsc3Settings);
    vector<FrontendStatusType> atsc3_fetypes;
    atsc3_fetypes.push_back(FrontendStatusType::DEMOD_LOCK);
    FrontendStatus atsc3_status;
    atsc3_status.isDemodLocked(true);
    vector<FrontendStatus> atsc3_vstatuses;
    atsc3_vstatuses.push_back(atsc3_status);
    frontendArray[ATSC3].tuneStatusTypes = atsc3_fetypes;
    frontendArray[ATSC3].expectTuneStatuses = atsc3_vstatuses;
    frontendArray[ATSC3].isSoftwareFe = false;
...
inline void initFilterConfig() {
    ...
    filterArray[TS_VIDEO0].settings.ts().tpid = 49; //48; //49;
    ...
    filterArray[TS_AUDIO0].settings.ts().tpid = 52; //64; //52;
    ...
    filterArray[TS_SECTION0].settings.ts().tpid = 0x0;
    filterArray[TS_SECTION0].settings.ts().filterSettings.section({
            .isRepeat = true,
            .isRaw = false,
    });
    
    // TS_SECTION1 filter setting
    filterArray[TS_SECTION1].type.mainType = DemuxFilterMainType::TS;
    filterArray[TS_SECTION1].type.subType.tsFilterType(DemuxTsFilterType::SECTION);
    filterArray[TS_SECTION1].bufferSize = FMQ_SIZE_16M;
    filterArray[TS_SECTION1].settings.ts().tpid = 0x1ffb;
    filterArray[TS_SECTION1].settings.ts().filterSettings.section({
            .isRepeat = true,
            .isRaw = false,
    });
    DemuxFilterSectionSettings::Condition::TableInfo tabledata1 {
        .tableId = 0xc8,
        .version = 0,
    };
    filterArray[TS_SECTION1].settings.ts().filterSettings.section().condition.tableInfo(tabledata1);

    
    // TS_SECTION2 filter setting
    filterArray[TS_SECTION2].type.mainType = DemuxFilterMainType::TS;
    filterArray[TS_SECTION2].type.subType.tsFilterType(DemuxTsFilterType::SECTION);
    filterArray[TS_SECTION2].bufferSize = FMQ_SIZE_16M;
    filterArray[TS_SECTION2].settings.ts().tpid = 0x1e01;
    filterArray[TS_SECTION2].settings.ts().filterSettings.section({
            .isRepeat = true,
            .isRaw = false,
    });
    DemuxFilterSectionSettings::Condition::TableInfo tabledata2 {
        .tableId = 0xcc,
        .version = 0,
    };
    filterArray[TS_SECTION2].settings.ts().filterSettings.section().condition.tableInfo(tabledata2);
