...
void TunerBroadcastHidlTest::broadcastMultiFilterTest(
                                       FilterConfig filterConf, 
                                       FilterConfig filterConf1, 
                                       FilterConfig filterConf2, 
                                       FrontendConfig frontendConf) {
    uint32_t feId;
    uint32_t demuxId;
    sp<IDemux> demux;
    uint32_t filterId, filterId1, filterId2;
...
void TunerBroadcastHidlTest::broadcastAVFilterTest1(
                                                       FilterConfig filterConf1,
                                                       FilterConfig filterConf2,
                                                       FrontendConfig frontendConf) {
    uint32_t feId;
...
int main(int argc, char **argv) {
    ...
    if (argc != 3) {
        ...
        case_id = atoi(argv[1]);
        frequency = 1000000 * atoi(argv[2]);
    }
    
    ALOGV("[%s:%d] @@tunerTester start case_id= %d, frequency= %d ", __FUNCTION__, __LINE__, case_id, frequency);
    
    android::TunerBroadcastHidlTest aBroadcastTest;
    aBroadcastTest.SetUp();
    ...
    if (case_id == 0) {
        ...
        atsc3Settings.frequency = frequency;
        frontendConf.settings.atsc3(atsc3Settings);
    ...
    ALOGV("[%s:%d] @@tunerTester end~", __FUNCTION__, __LINE__);
    return 0;
}
