...
#define ASSERT_TRUE_PTR(a)	if (a == nullptr) assert(0);
    ...
    virtual Return<void> onRecordStatus(DemuxFilterStatus status) override {
        ...
        switch (status) {
            ...
            case DemuxFilterStatus::OVERFLOW:
                ...
                EXPECT_TRUE_PTR(mDvr);// << "Dvr callback is not set with an IDvr";
                if (mDvr) {
...
class DvrTests {
  ...
  protected:
    static AssertionResult failure() { return false; }

    static AssertionResult success() { return true; }
