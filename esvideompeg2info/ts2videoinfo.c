...
#define RTK_TS_PACKET_SIZE (TS_PACKET_SIZE + TIME_STAMP_SIZE)
#define MAX_PID 8192
#define PES_PACKET_SIZE (3*1024*1024)

#define PID(p)            ( ( (((int)p[1]) & 0x1F) << 8 ) | p[2] )
#define START_UNIT(p)  ( ( p[1] & 0x40 ) >> 6 )
...
int main(int argc, char *argv[])
{
    ...
    //init ts buffer
    pTsPacket = malloc(RTK_TS_PACKET_SIZE);
    if (pTsPacket == NULL) {
    ...
    while(1) {
        ...
        /* read packets */
        byte_read = fread(pTsPacket, 1, RTK_TS_PACKET_SIZE, fd_ts);

        //fprintf(stderr, "{byte_read, tsPacketOffset}={%d, %d} \n", byte_read, tsPacketOffset);

        /* check packets pid */
        if (byte_read > 0) {
            pCurrent = pTsPacket + TIME_STAMP_SIZE;

            if (payload_pid == PID(pCurrent)) {
                ...
                if (temp == 0) {
                    ...
                    ts_header_size += AF_LENGTH(pCurrent) + 1;
                }

                if ((is_pes_start == 0) && (1 == START_UNIT(pCurrent))) {
                    is_pes_start = 1;
                ...
                } else if ((is_pes_start == 1) && (0 == START_UNIT(pCurrent))) {
                    memcpy(pPesPacket + pes_len, pCurrent + ts_header_size, TS_PACKET_SIZE - ts_header_size);
                    pes_len += TS_PACKET_SIZE - ts_header_size;
                } else if ((is_pes_start == 1) && (1 == START_UNIT(pCurrent))) {
